...
parser = OptionParser(version=
                      "Jabber Weather Transport: " + version)
parser.add_option('-c', '--config', metavar='FILE', dest='config', action='store',
                  help="Read config from custom file")

parser.add_option('-d', '--database', metavar = 'DATABASE', dest='citydb', action = 'store',
                  help="Specify custom DB")

(options, args) = parser.parse_args()
config =  options.config if options.config else 'config.xml'
citydb =  options.citydb if options.citydb else 'city.db'

config = os.path.abspath(os.path.dirname(sys.argv[0]))+"/"+config
citydb = os.path.abspath(os.path.dirname(sys.argv[0]))+"/"+citydb
