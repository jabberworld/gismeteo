...
parser = OptionParser(version="Jabber Weather Transport: " + version)
parser.add_option('-c', '--config', metavar='config.xml', dest='config', action='store', default='config.xml',
                  help="Read config from custom file [default: %default]")

parser.add_option('-d', '--database', metavar='city.db', dest='citydb', action='store', default='city.db',
                  help="Specify custom city DB [default: %default]")

(options, args) = parser.parse_args()

config = os.path.abspath(os.path.dirname(sys.argv[0]))+"/"+options.config
citydb = os.path.abspath(os.path.dirname(sys.argv[0]))+"/"+options.citydb
